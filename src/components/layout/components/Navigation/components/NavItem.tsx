...
const NavItem: React.FC<Props> = ({ items, href, setState }) => {
        ...
        <li key={index} className="relative">
          ...
          </Link>
          {
            ...
            item.children && item.children.length > 0 && (
              <div
                className={`absolute left-full top-0 ml min-w-[10rem] w-max bg-white border border-gray-200 rounded-xl shadow-lg opacity-0 ${item.isShow ? "visible opacity-100" : "invisible"} transition-all duration-300 z-50`}
                onMouseEnter={() => triggerShow(item.id || "", true)}
              >
