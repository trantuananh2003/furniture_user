...
const ProductDetailPage: React.FC = () => {
  ...
  const LoadRecommendProduct = async () => {
    try {
      ...
      const result = await clientAPI
        ...
        .find(`pageSize=${10}&pageCurrent=${1}`);
  ...
  const LoadSameProduct = async () => {
    try {
      setIsLoading(true);

      const result = await clientAPI
        .service("client/products/sameproduct")
        .find(`pageSize=${4}&pageCurrent=${1}&productSlug=${slug}`);

      setDataSameProductRecommend(result.result);
      setIsLoading(false);
    } catch (error) {
      console.error("Error during signup", error);
    }
  ...
  const AddProductToCart = async (cartItem: FormData) => {
    try {
      ...
      // Reload cart data and update Redux store
      const cartResponse: ApiResponse = await clientAPI.service("carts").find();
        ...
        <div className="flex-1 sm:max-w-[50%] max-w-[90%]">
          ...
          {dataProduct !== undefined && (
            <div className="flex flex-row items-center sm:justify-start justify-center rounded-3xl gap-4">
              <button
                className="font-semibold font-sans rounded-3xl bg-[#da684c] uppercase text-white py-2 px-4 hover:bg-[#f89075] cursor-not-allowed"
                // onClick={triggerBuyNow}
              >
                Mua ngay
              </button>
              <button
                className="font-sans uppercase border rounded-3xl border-gray-950 py-2 px-4 hover:bg-gray-400 cursor-pointer"
